...
//        //suppliers
//        syncDescription = "Used to sync suppliers from sun to my inventory daily.";
...
//        syncJobTypeRepo.save(consumptionSyncType);
        ...
        // Cost of Goods
        syncDescription = "Used to sync Cost of Goods from oracle hospitality reports to sun daily.";
        Configuration costOfGoodsConfig = new Configuration();
        costOfGoodsConfig.consumptionConfiguration = new ConsumptionConfiguration();
        if (account.getERD().equals(Constants.SUN_ERD) || account.getERD().equals(Constants.EXPORT_TO_SUN_ERD)) {
            costOfGoodsConfig.inforConfiguration = new InforConfiguration();
        }

        SyncJobType CostOfGoodsSyncType = new SyncJobType(8, Constants.COST_OF_GOODS, syncDescription, "/costOfGoods",
                new Date(), costOfGoodsConfig, account.getId());
        syncJobTypeRepo.save(CostOfGoodsSyncType);
...
//
//        // OPERA Reservation
//        syncDescription = "Used to sync reservation from opera.";
//        Configuration wlsIntegration = new Configuration();
//        wlsIntegration.supplierConfiguration = new SupplierConfiguration();
//
//        supplierSyncType = new SyncJobType(1, Constants.wLsIntegration, syncDescription, "/2wLsIntegration",
