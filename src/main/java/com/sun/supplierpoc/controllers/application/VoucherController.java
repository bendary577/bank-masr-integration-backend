...
public class VoucherController {
    ...
    @GetMapping("/getByID")
    public ResponseEntity<?> getVoucherById(@RequestParam("voucherId") String voucherId,
                                            Principal principal) {
        Response response = new Response();
        try {
            User user = (User) ((OAuth2Authentication) principal).getUserAuthentication().getPrincipal();
            if (user != null) {
                ...
                if (account != null) {
                    ...
                    response = voucherService.getVoucherById(account, voucherId);
    ...
    public void exportExcelSheet(@RequestBody Voucher voucher,
        ...
        if (accountOptional.isPresent()) {
            ...
            voucherCodePDFGenerator.generatePdfReport(account, voucher, httpServletResponse);
